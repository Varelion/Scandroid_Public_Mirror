...
const { Some_BioAndroid_COLOR, Some_BioAndroid_ICON_URL, DEV_SERVER_URL, Some_BioAndroid_RETIRE_COLOR, TESTING_SERVER_ID, LOGGING_CHANNEL_ID, ERROR_CHANNEL_ID, COMBAT_MULTIPLIER } = require("./config.json");
...
------
*/
function awardXPs(startingXp, cxp, levels) {
	for (; cxp > 0; cxp--) {
...
function getXp(wordCount, roleBonus, channelXpPerPost, xpPerPostDivisor, xpPerPostFormula, IS_COMBAT) {
	let IS_THIS_A_COMBAT_POST_MULTIPLIER = IS_COMBAT ? COMBAT_MULTIPLIER : 1; // Checks if combat is true. If it is true, you get 0.15 multiplier, if it's not, you get 1 multiplier
	switch (xpPerPostFormula) {
		case "exponential":
			// console.log(((channelXpPerPost + wordCount / xpPerPostDivisor) * (1 + wordCount / xpPerPostDivisor) * roleBonus) * IS_THIS_A_COMBAT_POST_MULTIPLIER)

			return ((channelXpPerPost + wordCount / xpPerPostDivisor) * (1 + wordCount / xpPerPostDivisor) * roleBonus) * IS_THIS_A_COMBAT_POST_MULTIPLIER;
		case "flat":
			return (channelXpPerPost * roleBonus) * IS_THIS_A_COMBAT_POST_MULTIPLIER;
		case "linear":
			return ((channelXpPerPost + wordCount / xpPerPostDivisor) * roleBonus) * IS_THIS_A_COMBAT_POST_MULTIPLIER;
	}
	return 0;
}
...
function buildCharacterEmbed(guildService, player, characterObj) {
	...
	*/
	...
	let characterEmbed = new EmbedBuilder()
		...
		.setThumbnail((characterObj["picture_url"] != "" && characterObj["picture_url"] !== "null") ? characterObj["picture_url"] : Some_BioAndroid_ICON_URL)
		.setFields(
...
async function logError(interaction, error) {
	...
	const logErrorEmbed = new EmbedBuilder()
		...
		.setTimestamp()
		.setColor(Some_BioAndroid_RETIRE_COLOR)
		.setThumbnail(`${interaction.client.user.avatarURL()}`)
	// ADDING FIELDS FOR EACH OF THE OPTIONS PASSED THROUGH
	for (const option of interaction.options._hoistedOptions) {

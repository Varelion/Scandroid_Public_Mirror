...
module.exports = {
    ...
    async execute(guildService, interaction) {
        ...
        */
		...
		let approveMessage = interaction.options.getString("approve_message") || null;
		approveMessage = approveMessage ? approveMessage.replace(/\'/g, "''") : approveMessage;

		let approveLevel = interaction.options.getInteger("approve_level") || null;

		let levelUpMessage = interaction.options.getString("level_up_message") || null;
		levelUpMessage = levelUpMessage ? levelUpMessage.replace(/\'/g, "''") : levelUpMessage;

		let levelUpChannel = interaction.options.getChannel("level_up_channel") || null;

		let moderationRole = interaction.options.getRole("moderation_role") || null;

		let numberOfCharacters = interaction.options.getInteger("number_of_characters") || null;

		let roleBonus = interaction.options.getString("role_bonus") || null;

		let tier1Role = interaction.options.getRole("tier_1_role") || null;

		let tier2Role = interaction.options.getRole("tier_2_role") || null;

		let tier3Role = interaction.options.getRole("tier_3_role") || null;

		let tier4Role = interaction.options.getRole("tier_4_role") || null;

		let allowPlayerManageXp = interaction.options.getString("allow_player_manage_xp") || null;

		let xpFreezeRole = interaction.options.getRole("xp_freeze_role") || null;

		let xpShareRole = interaction.options.getRole("xp_share_role") || null;

		let xpPerPostDivisor = interaction.options.getInteger("xp_per_post_divisor") || null;

		let xpPerPostFormula = interaction.options.getString("xp_per_post_formula") || null;
